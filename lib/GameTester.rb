...
class GameTester
  ...
  def manageDiscardTreasures (visible, aPlayer)
    ...
    begin #Se descartan tesoros hasta que se vuelve al men̼ anterior
      if visible then
        howMany = showTreasures("Elige tesoros visibles para descartar", aPlayer.visible_treasures(), true)
      else 
        howMany = showTreasures("Elige tesoros ocultos para descartar", aPlayer.hidden_treasures(), true)
      end
      option = getTreasure (howMany)
      if (option > -1) then 
        if visible then
          @game.discard_visible_treasures ([aPlayer.visible_treasures().at(option)])
        else
          @game.discard_hidden_treasures ([aPlayer.hidden_treasures().at(option)])          
        end
      end
    end while (option != -1)  
  end
  ...
  def manageMakeTreasureVisible (aPlayer)
    ...
    begin #Se hacen tesoros visibles hasta que se vuelve al men̼ anterior
      howMany = showTreasures("Elige tesoros para intentar hacerlos visibles", aPlayer.hidden_treasures(), true)
      option = getTreasure (howMany);
      if (option > -1) then
...
#        gets
      when Command::ShowVisibleTreasure then
        showTreasures("Esta es tu lista de tesoros visibles", aPlayer.visible_treasures(), false)
#        puts "pulsa enter para seguir"
#        gets
      when Command::ShowHiddenTreasure then
        showTreasures("Esta es tu lista de tesoros ocultos", aPlayer.hidden_treasures(), false)
#        puts "pulsa enter para seguir"
#        gets
